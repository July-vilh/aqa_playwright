...
test.describe("[API] [Customers] [Create]", () => {
  ...
  let id = "";
  let token = "";
  test("Create cusomer with smoke data", async ({ request }) => {
    ...
    const token = headers["authorization"]; //тк в этом ключе в headers респонса хранится токен
    ...
    expect.soft({ ...customerData }).toMatchObject(_.omit(customerBody.Customer, "_id", "createdOn")); //разложение в пустой объект, убираем лишнюю типизацию + omit (lodash)
