...
abstract class FilterModal extends SalesPortalPage {
  readonly uniqueElement = this.page.locator("div.modal");
  readonly title = this.uniqueElement.locator(".modal-title");
  readonly applyButton = this.uniqueElement.getByRole("button", {
    name: "Apply",
  });
  readonly clearFiltersButton = this.uniqueElement.getByRole("button", {
    name: "Clear Filters ",
  });
  ...
  readonly checkbox = (name: string) =>
    this.uniqueElement.locator(`input[value="${name}"]`);
